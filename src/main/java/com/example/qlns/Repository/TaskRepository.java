...
public interface TaskRepository extends JpaRepository<Task, Long> {
    @org.springframework.data.jpa.repository.EntityGraph(attributePaths = {"assignedTo", "assignedBy"})
    ...
    @org.springframework.data.jpa.repository.EntityGraph(attributePaths = {"assignedTo", "assignedBy"})
    @Query("SELECT t FROM Task t WHERE t.assignedTo.department.id = :deptId OR t.assignedBy.department.id = :deptId")
    ...
    @Query("""
        ...
        WHERE (t.assignedTo.department.id = :deptId OR t.assignedBy.department.id = :deptId) 
          AND t.status = :status
    """)
    long countByDepartmentAndStatus(@Param("deptId") Long deptId, @Param("status") TaskStatus status);
}
